...
describe file('/etc/monitrc'), :if => os[:family] == 'redhat' && not os[:release].match(/^8\./)  do
  it { should be_file }
end
describe file('/etc/monit.d/sshd'), :if => os[:family] == 'redhat' && not os[:release].match(/^8\./) do
  it { should be_file }
end

describe command("monit status (#{os[:family]}, #{os[:release]})"), :if => os[:family] != 'redhat' || not os[:release].match(/^8\./) do
